...
class Articles extends Component {
  ...
  fetchArticle(nid) {
    ...
    var self = this;
    this.serverRequest = axios.get('http://d8.decoupled.localhost:8000/node/' + id + '?_format=json')
    .then(function(result){
      var body = result.data.body["0"].value;
      self.setState({
        article_title: result.data.title["0"].value,
        article_body: body.replace('/sites/default/files', 'http://d8.decoupled.localhost:8000/sites/default/files')
      });
    })
  }
  ...
  fetchArticleTitles() {
    var self = this;
    this.serverRequest = axios.get('http://d8.decoupled.localhost:8000/api/v1/articles/list')
    .then(function(result){
      self.setState({
